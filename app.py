...
# DATABASE_URL Unterstützung (Render Postgres) + Fallback SQLite
_db_url = os.getenv('DATABASE_URL', 'sqlite:///dogcrm.db')
# Render/Heroku geben oft "postgres://" → von SQLAlchemy erwartet "postgresql+psycopg2://"
...
class AdminUser(UserMixin, db.Model):
    ...
    is_admin = db.Column(db.Boolean, default=True)  # einfaches Rollen-Flag
...
def init_admin():
    ...
    user = AdminUser(username=u, is_admin=True)
    user.set_password(p)
    db.session.add(user)
    db.session.commit()
...
def send_booking_email(slot: Slot):
    ...
    notify = os.getenv('SMTP_NOTIFY')  # optional: interne Benachrichtigung (kommagetrennt)
...
def booking_submit(slot_id):
    ...
    # Email an Kunden (und optional interne BCC)
    try:
        send_booking_email(slot)
    except Exception as e:
        # Nicht hart failen – nur loggen (hier minimal)
...
# ---------------- Admin: Views (geschützt mit @admin_required) ----------------
# Availability
...
# Alias, weil Templates availability_index nutzen
