...
# CSRF-Helper als Jinja-Funktion verfügbar machen → {{ csrf_token() }}
@app.context_processor
def inject_csrf():
...
def send_booking_email(name, email_to, date_str, time_str):
    ...
    # Nur senden, wenn ein gültiger Host konfiguriert ist
    if not host or host in ('disable', 'none', 'off'):
        return
...
# Öffentliche Buchung zeigt verfügbare (nicht gebuchte) Availabilities
