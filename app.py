...
def booking_submit(slot_id):
    ...
    if not slot:
        abort(404)
    slot.customer_name = request.form["name"]
    slot.customer_email = request.form["email"]
    slot.customer_phone = request.form["phone"]
    slot.booked = True
    db.session.commit()
...
# Alias, weil Template availability_index nutzt
