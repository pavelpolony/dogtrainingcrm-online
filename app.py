from flask import (
    ...
    make_response, g, abort, session, flash, jsonify
)
...
# CSRF-Helper als Jinja-Funktion → {{ csrf_token() }}
@app.context_processor
def inject_csrf():
    return dict(csrf_token=lambda: ensure_csrf_token())
...
class Availability(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    start = db.Column(db.String(50))  # ISO-String aus <input type="datetime-local">
...
def send_booking_email(name, email_to, date_str, time_str):
    ...
    # Nur senden, wenn ein sinnvoller Host konfiguriert ist
    if not host or host.lower() in ('disable', 'none', 'off'):
        return
...
# Öffentliche Buchung zeigt verfügbare Availabilities
...
def book_slot(avail_id):
    ...
    return render_template("book_slot.html", a=a, date_str=date_str, time_str=time_str)
...
def book_submit(avail_id):
    ...
    # Termin anlegen + Availability sperren
    ap = Appointment(availability_id=a.id, customer_id=c.id, status="confirmed")
    db.session.add(ap)
    a.booked = True
    db.session.commit()

    # Mail (optional, falls SMTP konfiguriert)
    try:
        parts = (a.start or "").split("T")
        date_str = parts[0] if parts else ""
        time_str = parts[1] if len(parts) > 1 else ""
...
# ---- Customers: NEW (GET + POST)
@app.route("/admin/customers/new", methods=["GET", "POST"], endpoint="new_customer")
@admin_required
def customers_new():
    if request.method == "POST":
        if not verify_csrf(request.form.get("csrf_token")):
...
def settings_page():
    ...
    if request.method == 'POST':
        ...
        if not prefix or len(prefix) > 20:
            flash(TRANSLATIONS[g.lang]['invalid_prefix'], 'error'); ok = False

        try:
            next_no = int(next_no_raw)
            if next_no < 1: raise ValueError()
        except Exception:
            flash(TRANSLATIONS[g.lang]['invalid_next'], 'error'); ok = False
            next_no = s.next_number

        try:
            vat = float(vat_raw.replace(',', '.'))
            if vat < 0 or vat > 100: raise ValueError()
        except Exception:
            flash(TRANSLATIONS[g.lang]['invalid_vat'], 'error'); ok = False
            vat = s.vat_rate
...
# ---------- REST-API für Mobile App (Flutter) ----------
API_TOKENS = set()  # einfache In-Memory-Tokenliste; für Produktion: DB/Redis
...
def api_availabilities():
    only_free = request.args.get("only_free") in ("1","true","True","yes")
    q = Availability.query
    if only_free:
        q = q.filter_by(booked=False)
    avails = q.order_by(Availability.start.asc()).all()
    return jsonify([
        {
            "id": a.id,
            "start": a.start,
...
@app.route("/api/appointments/<int:ap_id>/done", methods=["POST"])
@require_bearer_token
def api_appointment_done(ap_id):
    ap = Appointment.query.get(ap_id)
    if not ap:
